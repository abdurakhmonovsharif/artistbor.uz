export type OrderUiStatusKey = "pending" | "confirmed" | "completed" | "cancelled" | "unknown";
...
function normalizeStatus(status: unknown, label: unknown) {
  ...
  if (numericStatus === 20) return "pending";
  ...
  if (hasAny(text, ["pending", "payment", "unpaid", "waiting", "review", "new", "kutil"])) return "pending";
